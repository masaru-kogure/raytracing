...
class lunge_kutta4:
    def main_lunge(dudx, dvdx, dudy, dvdy, u, v, w, NF, H, gome, k, l, lon, lat, time, z, lonM, latM, timeM, zM, phi, ram, dt, dlat, dlon, Cs2 = None):
        ...
        if np.isnan(zM):
            print("dame")
        dudx1, dudy1, dvdx1, dvdy1, u1, v1, w1, NF1, H1, Cs21 = lunge_kutta4.data_interpo(dudx, dvdx, dudy, dvdy, u, v, w, NF, H, lon, lat, time, z, lonM, latM, timeM, zM, dlat, dlon, Cs2 = Cs2)
        if np.isnan(zM):
            print("dame")
        step = 1
        zM2, lonM2, latM2, timeM2, k2, l2, ug1, vg1, wg1, ome1, dk1 ,dl1 = lunge_kutta4.lunge_step(H1, gome, k, u1, l, v1, latM, lonM, NF1, w1, dudx1, dvdx1, dudy1, dvdy1, zM, timeM, step, dt, Cs2 = Cs21)
        if np.isnan(zM2):
            print("dame") 
        dudx2, dudy2, dvdx2, dvdy2, u2, v2, w2, NF2, H2, Cs22 = lunge_kutta4.data_interpo(dudx, dvdx, dudy, dvdy, u, v, w, NF, H, lon, lat, time, z, lonM2, latM2, timeM2, zM2, dlat, dlon, Cs2 = Cs2)

        step = 2
        zM3, lonM3, latM3, timeM3, k3, l3, ug2, vg2, wg2, ome2, dk2 ,dl2 = lunge_kutta4.lunge_step(H2, gome, k2, u2, l2, v2, latM2, lonM2, NF2, w2, dudx2, dvdx2, dudy2, dvdy2, zM2, timeM2, step, dt, Cs2 = Cs22)
        if np.isnan(zM3):
            print("dame")  
        dudx3, dudy3, dvdx3, dvdy3, u3, v3, w3, NF3, H3, Cs23 = lunge_kutta4.data_interpo(dudx, dvdx, dudy, dvdy, u, v, w, NF, H, lon, lat, time, z, lonM3, latM3, timeM3, zM3, dlat, dlon, Cs2 = Cs2)

        step = 3
        zM4, lonM4, latM4, timeM4, k4, l4, ug3, vg3, wg3, ome3, dk3 ,dl3 = lunge_kutta4.lunge_step(H3, gome, k3, u3, l3, v3, latM3, lonM3, NF3, w3, dudx3, dvdx3, dudy3, dvdy3, zM3, timeM3, step, dt, Cs2 = Cs23)
        if np.isnan(zM4):
            print("dame")  
        dudx4, dudy4, dvdx4, dvdy4, u4, v4, w4, NF4, H4, Cs24 = lunge_kutta4.data_interpo(dudx, dvdx, dudy, dvdy, u, v, w, NF, H, lon, lat, time, z, lonM4, latM4, timeM4, zM4, dlat, dlon, Cs2 = Cs2)
        step = 4
        ug4, vg4, wg4, ome4, dk4 ,dl4 = lunge_kutta4.lunge_step(H4, gome, k4, u4, l4, v4, latM4, lonM4, NF4, w4, dudx4, dvdx4, dudy4, dvdy4, zM4, timeM4, step, dt, Cs2 = Cs24)
    ...
    def data_interpo(dudx, dvdx, dudy, dvdy, u, v, w, NF, H, lon, lat, time, z, lonM, latM, timeM, zM, dlat, dlon, Cs2 = None):
        ...
        zmin = np.nanmin(np.where(z[np.argmin(np.abs(np.round(time - np.min(timeM)))), :, int(np.round((latM + 90)/dlat)), int(np.round(lonM/dlon))] <= zM)) + 1
        zmax = np.nanmax(np.where(z[np.argmin(np.abs(np.round(time - np.min(timeM)))), :, int(np.round((latM + 90)/dlat)), int(np.round(lonM/dlon))] > zM)) - 1
        if zmax < 0:
            zmax = 0
        num = zmin - zmax + 1 
        dudxM1 = np.zeros(num)
        ...
        for i_high in range(zmax,zmin+1):
            dudxM1[i_high - zmax] = interpn([time, 90 + lat, lon], dudx[:,i_high,:,:], [timeM, 90 + latM, lonM])
            dvdxM1[i_high - zmax] = interpn([time, 90 + lat, lon], dvdx[:,i_high,:,:], [timeM, 90 + latM, lonM])
            dudyM1[i_high - zmax] = interpn([time, 90 + lat, lon], dudy[:,i_high,:,:], [timeM, 90 + latM, lonM])
            dvdyM1[i_high - zmax] = interpn([time, 90 + lat, lon], dvdy[:,i_high,:,:], [timeM, 90 + latM, lonM])
            uM1[i_high - zmax] = interpn([time, 90 + lat, lon], u[:,i_high,:,:], [timeM, 90 + latM, lonM])
            vM1[i_high - zmax] = interpn([time, 90 + lat, lon], v[:,i_high,:,:], [timeM, 90 + latM, lonM])
            wM1[i_high - zmax] = interpn([time, 90 + lat, lon], w[:,i_high,:,:], [timeM, 90 + latM, lonM])
            NFM1[i_high - zmax] = interpn([time, 90 + lat, lon], NF[:,i_high,:,:], [timeM, 90 + latM, lonM])
            HM1[i_high - zmax] = interpn([time, 90 + lat, lon], H[:,i_high,:,:], [timeM, 90 + latM, lonM])
            zM1[i_high - zmax] = interpn([time, 90 + lat, lon], z[:,i_high,:,:], [timeM, 90 + latM, lonM])
            if Cs2 is not None:
                Cs2M1[i_high - zmax] = interpn([time, 90 + lat, lon], Cs2[:,i_high,:,:], [timeM, 90 + latM, lonM])

          
        dudxM = np.interp(zM, np.flip(zM1), np.flip(dudxM1)) 
        dvdxM = np.interp(zM, np.flip(zM1), np.flip(dvdxM1)) 
        dudyM = np.interp(zM, np.flip(zM1), np.flip(dudyM1)) 
        dvdyM = np.interp(zM, np.flip(zM1), np.flip(dvdyM1))   
        uM = np.interp(zM, np.flip(zM1), np.flip(uM1))  
        vM = np.interp(zM, np.flip(zM1), np.flip(vM1))
        wM = np.interp(zM, np.flip(zM1), np.flip(wM1)) 
        NFM = np.interp(zM, np.flip(zM1), np.flip(NFM1)) 
        HM = np.interp(zM, np.flip(zM1), np.flip(HM1))
        if Cs2 is not None:
            Cs2M = np.interp(zM, np.flip(zM1), np.flip(Cs2M1))
        return dudxM, dvdxM, dudyM, dvdyM, uM, vM, wM, NFM, HM, Cs2M
   
    def lunge_step(H, gome, k, u, l, v, latM, lonM, NF, w, dudx, dvdx, dudy, dvdy, zM, timeM, step, dt, Cs2 = None):
        ...
        m = -np.sqrt(meteo_para.cal_dis_ral( k, l, ome, f, NF, H, cs2 = Cs2))
        dk = -(k * dudx + l * dvdx)
        dl = -(k * dudy + l * dvdy + b * f/ome)
        ug, vg, wg = meteo_para.group_velocity( k, l, m, H, NF, ome, w, f, u, v, cs2 = Cs2)
        #-------second step------------------
        if step == 1 or step == 2:
            zM2 = zM + dt_2 * wg
