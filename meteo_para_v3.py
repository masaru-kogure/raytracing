##################################################
## Description
## This program calculates meteorological parameters for the ray-tracing model.
...
from derivation import derivation
        ...
        #Cp = 1004. ;[J/(K*kg)]
        #R = 287. ;[J/(K*kg)]
        #An Introduction to Dynamic Meteorology, p.491
        #ps = 1000. ;[hpa]
        #-------------constant----------------------
...
class meteo_para:
    ...
    def cal_dis_ral( k, l, omei, f, NFULL, H, cs2 = None, m = 0):
        if m == 0:
            if cs2 is None:
    ...
    def group_velocity( k, l, m, H, NF, omei, w, f, u, v, cs2 = None):
        if cs2 is None:
            deno = (k**2 + l**2 + m**2 + 1/(4 * H**2)) * omei
        else:
            deno = (k**2 + l**2 + m**2 + 1/(4 * H**2) + (f ** 2 - 2 * omei ** 2)/cs2) * omei
        CONS = (NF -omei**2)/deno
        ug = u + CONS * k
        vg = v + CONS * l 
    ...
    def intrinsic_group_velocity( k, l, m, H, NF, omei, f, cs2 = None):
        if cs2 is None:
            deno = (k**2 + l**2 + m**2 + 1/(4 * H**2)) * omei
        else:
            deno = (k**2 + l**2 + m**2 + 1/(4 * H**2) + (f ** 2 - 2 * omei ** 2)/cs2) * omei
    ...
    def gas_const(N2 = 0.75527, O = 0, O2 = 0.23143, H2O = 0, NO = 0):#, CO2 = 0.0456):
        R = (N2/28.01 + O/18 + O2/32 + H2O/18.02 + NO/32 ) * R0#+ CO2/44.01)
    ...
    def scale_p_h(T, R = 287.07, g = 9.80665):
        H = T * R/g
        return H
    
    def scale_d_h(rho, z):
        H = -1/derivation.interpol_diff(z, np.log(rho))
        return H
    ...
    def sound_wave_speed(t, Cp = 1004., R = 287.07, Cv = 718):
        cs2 = Cp/(Cv) * R * t
        return cs2
